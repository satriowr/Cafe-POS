...
@foreach ($menus as $menu)
    @php
        $notAvailable = $menu->is_available == 0;
    @endphp
    <div class="col d-flex justify-content-center">
        <div class="card shadow-sm border-0 p-2 h-100 w-100 {{ $notAvailable ? 'bg-light' : '' }}"
            style="max-width: 200px; {{ $notAvailable ? 'opacity: 0.7; filter: grayscale(0.2);' : '' }}">
            
            <div class="position-relative">
                <img src="{{ asset('storage/' . $menu->image) }}" alt="{{ $menu->name }}"
                    class="rounded {{ $notAvailable ? 'unavailable-img' : '' }}"
                    style="width: 100%; height: 120px; object-fit: cover; {{ $notAvailable ? 'opacity:0.5; filter: grayscale(0.6);' : '' }}">
                @if($notAvailable)
                    <div style="position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(160,160,160,0.4);border-radius:6px;"></div>
                @endif
            </div>
            ...
            <button class="btn mt-3 w-100 py-1"
                ...
                style="{{ $notAvailable ? 'background-color:#bdbdbd; color:#fff; border:none; cursor:not-allowed;' : 'background-color:#038447; color:white; border:none;' }}"
                {{ $notAvailable ? 'disabled' : '' }}>
                {{ $notAvailable ? 'Tidak Tersedia' : 'Tambah' }}
            </button>
        </div>
    </div>

    <!-- Modal tetap ada (optional: bisa juga di-nonaktifkan kalau menu tidak available) -->
            ...
            <form action="{{ route('user.cart.add', ['token' => request('token')]) }}" method="POST" class="w-100">
                ...
                <div class="modal-content rounded-top">
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-success w-100" {{ $notAvailable ? 'disabled' : '' }}>Tambah ke Keranjang</button>
                    </div>
                </div>
            </form>
        </div>
